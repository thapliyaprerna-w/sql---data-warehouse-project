...
=======================================================================================================================
Script purpose :
 this script create a table in 'silver' schema droping existing table if the already exist 
...
FROM(
SELECT *,
ROW_NUMBER() OVER(PARTITION BY cst_id ORDER BY cst_create_date DESC) AS FALG_RED
...
SELECT 
    ...
    CASE 
        ...
        WHEN UPPER(TRIM(prd_line)) = 'T' THEN 'Touring sales'
        ELSE 'N/A'
    END AS prd_line,
...
NOW() AS dwh_create_date   -- ✅ added missing column
